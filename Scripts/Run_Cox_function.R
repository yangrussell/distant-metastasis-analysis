...
require(preprocessCore)
	...
	# Print what's happening
	print("Extracted info split")

	# Extract DMFS_Time column as a numeric
	time= as.numeric(train_info[,paste("DMFS","_Time",sep="")])

	# Extract DMFS_Status column
	status=train_info[,paste("DMFS","_Status",sep="")]
	
	nsam=length(which(!is.na(time)&!is.na(status)))
	if (nsam<10) {
		cat(paste("# NSAMPLES_USED:",nsam,"\n"),file=project)
		cat(paste("ERROR: less than 10 samples with data\n"),file=project)
		quit()
	}
    
	# Make nn a sequence from 1 to the length of time
	nn=1:length(time)

	# Do univariate cox regression
	survres=unisurvcns(train,time,status,nn)  

	# Cbind annot and survres together
	survres=cbind(annot,survres)

	# Remove if the pval is na
	np=which(!is.na(survres$pval))
	pvals=survres$pval[np]

	# Calculate FDR_tool q-values (p-values adjuted for the false discovery rate)
	fdr.res=fdrtool(pvals,statistic="pvalue",plot=F)
	survres$FDRtool_qval=NA
	survres$FDRtool_lFDR=NA
	survres$FDRtool_qval[np]=fdr.res$qval
	survres$FDRtool_lFDR[np]=fdr.res$lfdr

	# Calculate Benjamini-Hochberg '95
	od=order(order(pvals))
	survres$BH95=NA
	survres$BH95[np]=length(np)*pvals/od

	# Calculate Storey q-value
	storey=qvalue(pvals)
	survres$Storey_qvalue=NA
	survres$Storey_qvalue[np]=storey$qvalues
	survres$mean_expr=mns
	survres$stdev_expr=sds
	survres$median_expr=mds
	
	
	# Write the survres and rnkres files
	cat(paste("# NSAMPLES_USED:",nsam,"\n"),file=project)
	write.table(survres,file=paste("PRECOG_DMFS/Results/",project,".","DMFS",".survres.tsv",sep=""),sep="\t",row.names=F,quote=F,append=F)
	rnkres=survres[,c(1,6)]
	write.table(rnkres,file=paste("PRECOG_DMFS/Results/",project,".","DMFS",".rnkres.tsv",sep=""),sep="\t",row.names=F,quote=F,col.names=F)
